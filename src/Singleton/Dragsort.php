        ...
        if (!empty($setting_items)) {
            ...
            foreach ($setting_items as $key=>$arr) {
                ...
                switch ($arr['type']) {
                    ...
                    break;
                    case 'crop':
                        $opt.='<a id="'.$key.'_'.$i.'" class="btn btn-primary" href="#" data-component="image-crop" data-ie9img="'.\LizusVitara\v_url(\get_bloginfo('url').'/ajax.php','action=ie9img').'" data-url="'.\LizusVitara\v_url(\get_bloginfo('url').'/ajax.php','action=crop_upload').'" data-width="'.$arr['crop_width'].'" data-height="'.$arr['crop_height'].'" data-size='.(3.5*1024*1024).'>'.@$arr['placeholder'].'<i class="icon-images"></i></a><textarea name="'.$key.'" data-id="'.$key.'_'.$i.'" data-name="dragsort_item" class="input textarea crop_textarea" rows=10 >'.@$data[$key].'</textarea>';
                    break;
                    default:
